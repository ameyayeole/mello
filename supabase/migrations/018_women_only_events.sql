-- ─────────────────────────────────────────────────────────────────────────────
-- 018: Female-only events. Adds events.women_only; only profiles with
-- gender = 'female' (and the host) can see these events — enforced in RLS and
-- in both feed functions. Both functions gain a women_only column in their
-- return shape, so they must be dropped first (CREATE OR REPLACE can't change
-- a RETURNS TABLE shape).
...
-- ─── RLS: gate women-only events at the row level ────────────────────────────
