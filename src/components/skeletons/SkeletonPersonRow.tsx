...
export default function SkeletonPersonRow({ count = 6 }: { count?: number }) {
  return (
    <View style={styles.list}>
      {Array.from({ length: count }, (_, i) => (
        <View key={i} style={styles.row}>
          <SkeletonBone circle size={AVATAR} />
          <View style={styles.text}>
            {/* Two widths, not one: a column of identical bars reads as a table
                rather than as names. */}
            <SkeletonBone w={i % 2 ? '52%' : '38%'} h={13} />
            <SkeletonBone w={i % 2 ? '34%' : '46%'} h={11} />
          </View>
        </View>
      ))}
    </View>
